...
```bash
OS=$(uname -s | tr '[:upper:]' '[:lower:]'); ARCH=$(uname -m); [ "$ARCH" = "x86_64" ] && ARCH=amd64; [ "$ARCH" = "arm64" ] || [ "$ARCH" = "aarch64" ] && ARCH=arm64; case "$OS" in linux) SUFFIX="linux-${ARCH}" ;; mingw*|msys*|cygwin*) SUFFIX="windows-amd64.exe" ;; *) echo "No prebuilt rustagent for $OS/$ARCH, build locally with: cd rust && cargo build --release"; exit 1 ;; esac; curl -fsSL "https://github.com/lloydzhou/bash-agent/releases/latest/download/rustagent-${SUFFIX}" -o ~/.local/bin/rustagent && chmod +x ~/.local/bin/rustagent
```
