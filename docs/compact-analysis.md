...
if (( DP_E_FIXED > 0 )); then
    ...
    E=$(( remaining > baseline / 2 ? remaining : baseline / 2 ))
else
    E=2
fi
```
...
⑤ QP × P_input × [(V+T)² - (V+K)²] / (M × 1e6)         质量改善增量收益（正项）
```
...
上下文越长，模型回答质量越差，需要重试的概率越大。压缩缩短上下文后的质量改善量为增量收益。

- **不压缩的衰减成本**：`QP × P_input × (V+T)² / (M × 1e6)`
- **压缩后的衰减成本**：`QP × P_input × (V+K)² / (M × 1e6)`
- **增量收益** = 不压缩成本 - 压缩后成本 = `QP × P_input × [(V+T)² - (V+K)²] / (M × 1e6)`
  - T = total_tokens（全部消息总 token 数），K = 保留的 token 数
  - 因为 T > K（有内容被压缩），所以增量收益恒为正

##### 默认值 QP=0.2 的推导依据

参考 "Lost in the Middle: How Language Models Use Long Contexts" (Liu et al., 2023) 等研究，LLM 在长上下文下的质量衰减近似如下：

| 上下文占比 | 质量衰减（论文数据） | ⑤ 应占 ① 比例 | 反推 QP |
|---|---|---|---|
| 30% | ~5% | 2.3% | 0.2 |
| 50% | ~10% | 6.4% | 0.2 |
| 80% | ~20% | 16% | 0.2~0.25 |

各场景下 QP=0.2 一致。对应的实际影响：

| 上下文占比 | ⑤ 增量收益（QP=0.2, p_input=3.0, M=200K, H=40K） | 占 ① 比例 | 含义 |
|---|---|---|---|
| 30% | $0.014 | 3.0% | 日常对话，几乎不影响决策 |
| 50% | $0.030 | 6.4% | 开始轻微作用 |
| 70% | $0.059 | 12.6% | 明显影响 |
| 90% | $0.098 | 20.8% | 接近上限，显著推动压缩 |

> 注：典型场景 R=40（E=8, L=5），H=40K 时 ①≈$0.47。

### 变量定义

| 变量 | 含义 | 获取方式 | 单位 |
|------|------|----------|------|
| E | 预期剩余用户输入轮数 | max(DP_BASELINE_E - current_turn, baseline/2) | 轮 |
...
| QP | 质量衰减系数（增量收益正项） | DP_QUALITY_PENALTY = 0.2 | 无量纲 |
...
假设会话第 8 轮（t=8），累积上下文 120k token，固定前缀 30k token。保留最近 3 轮消息（k=3），K=25k，H=120k-25k=65k（注：H=total-K，V 不参与 H 计算）。预估 E=4，L=3，R=12。S=500，c=1，r_t=0.8^2=0.64。β=0.03，avg=4k。M=200K，QP=0.2。
...
⑤ = 0.2 × 3.0 × [(5000+90000)² - (5000+25000)²] / (200000 × 1e6)
   = 0.6 × (9.025e9 - 9.0e8) / 2e11 = 0.6 × 8.125e9 / 2e11 = 0.0244

NetBenefit = 0.2145 - 0.0689 - 0.0287 - 0.00156 + 0.0244 = $0.1397 → 正收益，执行压缩
```
...
对每个 k (min_keep..NR) 计算 NetBenefit(k):
  ...
  8. ⑤ quality_savings = QP × P_input × [(V+T)² - (V+K)²] / (M × 1e6)
