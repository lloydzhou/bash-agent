...
/// agent 线程 broadcast(line) -> 每个客户端的 channel 收到 -> 客户端线程写 WS。
...
/// 行缓冲的 stdout writer：把 stream-json 每行事件转发给 WS 广播。
/// 缓存到换行符再发，避免一次 write 调用切碎 JSON。
...
impl WsWriter {
    pub(crate) fn new(hub: Arc<WsHub>) -> Self {
        Self {
            hub,
            pending: Vec::new(),
        }
...
fn handle_connection(
    ...
    if path == "/" {
        serve_static(&mut stream, index.as_bytes(), "text/html; charset=utf-8")?;
        return Ok(());
    }
    ...
    if path == "/ws" {
        ...
        client_loop(&mut ws, &hub, &input_tx, events_path.as_deref())?;
        return Ok(());
    }
...
fn client_loop(
    ...
    // 每次连接时读取最新的 events.jsonl 并回放最近 500 条，再注册为活跃监听者。
    // 这样页面刷新重连也能看到当前会话的全部历史（含启动后产生的事件）。
...
/// serve 服务器句柄：hub 用于构造 WsWriter，input_rx 接收用户输入行。
            ...
            match stream {
                Ok(stream) => {
                    ...
                    std::thread::spawn(move || {
                        let _ = handle_connection(stream, hub, input_tx, events_path, index);
                    });
                }
                Err(_) => break,
            }
