...
static RE_BASH_SYSTEM_PATH: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r#"(^|[\s"'`])(/etc|/usr|/bin|/sbin|/var|/library|/system|/dev)(/|[\s"'`]|$)"#)
        .expect("regex")
});
...
static RE_BASH_EXTERNAL_PATH: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r#"(^|[\s"'`])(~|\$home)(/|[\s"'`]|$)|(^|[\s"'`])/[A-Za-z0-9._-]"#).expect("regex")
});
...
fn wait_child_or_cancel(child_pid: u32, cancel_fd: i32, timeout: Duration) -> Result<WaitResult> {
    ...
    let timeout_nanos =
        timeout.as_secs().saturating_mul(1_000_000_000) as i64 + timeout.subsec_nanos() as i64;
...
fn wait_child_or_cancel(child_pid: u32, cancel_fd: i32, timeout: Duration) -> Result<WaitResult> {
    ...
    let cancelled = poll_fds.len() > 1 && (poll_fds[1].revents as i16 & libc::POLLIN as i16) != 0;
    ...
    if exited {
        ...
        bail!(
            "unexpected poll result: revents={:?}",
...
fn bash_scan_segment(mask: &mut u16, seg: &str, cwd: &str) {
    ...
    match seg {
        s if s == "sudo"
            ...
            || s.starts_with("halt")
            || s.starts_with("poweroff") =>
        {
            bash_add_mode(mask, 8, 1)
        }
        s if s.starts_with("mkfs")
            || s.starts_with("fdisk")
            || s.starts_with("diskutil")
            || s.starts_with("mount ")
            || s.starts_with("umount ") =>
        {
...
mod tests {
    ...
    fn bash_mode_classifier_cwd_aware() {
        ...
        let cases: Vec<(String, &str)> = cases
            .iter()
            .map(|(c, w)| {
                (
                    c.replace("SAMPLE_CWD", &cwd).replace("SAMPLE_HOME", &home),
                    *w,
                )
            })
            .collect();
    ...
    fn bash_mode_classifier_compound() {
        ...
        let cases: &[(&str, &str)] = &[
            (
                "echo hi > SAMPLE_CWD/test.txt && cat SAMPLE_CWD/test.txt",
                "0006",
            ),
        ...
        let cases: Vec<(String, &str)> = cases
            .iter()
            .map(|(c, w)| (c.replace("SAMPLE_CWD", &cwd), *w))
            .collect();
        for (cmd, want) in &cases {
            assert_eq!(classify_bash_required_mode(cmd), *want, "{cmd}");
        }
    ...
    fn bash_mode_allows_matches_bash() {
        ...
        for (allowed, required, want) in cases {
            assert_eq!(
                bash_mode_allows(allowed, required),
                want,
                "{allowed} {required}"
            );
        }
