...
> **最近一次调用输出速度统计**：终端标题栏新增输出速度，Bash / Go / Rust / C 统一完整传输计时及失败处理；修复 Linux 发布构建容器的软件源问题，保持 GLIBC 2.31 兼容基线。

### Added

- **终端标题栏输出速度**：新增 `S:…tok/s`，展示最近一次成功 LLM 调用的输出速度；四个运行时统一持久化 `last_call_speed_tok_per_sec`，兼容缺少该字段的旧会话。
- **速度统计回归测试**：补充 Go 单元测试、C 传输测试及跨运行时定向测试，覆盖流尾等待、失败终态、提前结束、零输出与重试等场景。

### Fixed

- **输出速度计时偏高（#87）**：Go / Rust / C 对齐 Bash，在完整传输结束后计算输出速度，计入协议终态之后的流尾等待；以输出 token 数除以读流耗时，排除建连、TLS 与首字节等待。Bash 使用 `curl -w` 提供的完整传输与首字节时间，移除额外时钟回退逻辑。
- **失败调用覆盖上次速度**：传输失败、中断或流提前结束时保留上次成功调用的速度；成功但零输出时清零。重试清理暂存的终态与用量，并保留整体读流计时窗口，避免使用失败尝试的残留数据。
- **Linux 发布构建软件源失效**：构建容器统一切换至 Ubuntu 20.04，解决 Debian 11 软件源归档引发的构建问题，继续保持 GLIBC 2.31 基线及兼容性检查。
