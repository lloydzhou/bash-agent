...
build_system_prompt() {
    ...
    core_rules=$'- MUST USE \"'"${locale}"$'\" FOR ALL OUTPUT!!! INCLUDING THINKING/REASONING!!! NEVER MIX LANGUAGES!!! Code, commands, and file content remain as-is.\n- Be concise and concrete. No pleasantries, no explanations unless asked. Raw results only.\n- Prefer safe, exact edits.\n- Report failures clearly.'
    echo "$core_rules" >> /dev/stderr
    environment="lang: ${locale}"$'\n'"pwd: ${PWD:-$(pwd)}"$'\n'"home: ${HOME}"$'\n'"platform: $(uname -s 2>/dev/null || echo unknown)"$'\n'"shell: ${SHELL:-unknown}"$'\n'"locale: MUST use \"${locale}\" for all output, including thinking/reasoning!"
