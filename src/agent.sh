...
build_system_prompt() {
    ...
    local lang_rule
    if [[ "$locale" == zh* ]]; then
        lang_rule="必须使用 ${locale} 进行所有输出，包括思考/推理。严禁混用语言。代码、命令和文件内容保持原样。"
    else
        lang_rule="MUST use ${locale} for all output, including thinking/reasoning. Never mix languages. Code, commands, and file content remain as-is."
    fi
    core_rules="${lang_rule}\n- Be concise and concrete. No pleasantries, no explanations unless asked. Raw results only.\n- Prefer safe, exact edits.\n- Report failures clearly."
